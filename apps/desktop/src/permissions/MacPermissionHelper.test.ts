...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/Control Plane (Nightly).app/Contents/MacOS/Control Plane",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/Control Plane.app/Contents/MacOS/Control Plane")).toBe(
      "/Applications/Control Plane.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(
      macAppBundlePath("/Applications/Control Plane.app/other/MacOS/Control Plane"),
    ).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/Control Plane (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/Control Plane (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged Control Plane icon is missing");
  expect(windows).toHaveLength(0);
});
