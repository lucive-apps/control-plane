...
<header>↑ Drag Control Plane into the list above</header>
<button id="app" draggable="true" aria-label="Drag Control Plane to System Settings, or click to reveal in Finder"><img src="${escapeHtml(icon)}" alt="" draggable="false">Control Plane</button>
</main></body></html>`;
}
...
export class MacPermissionHelper {
  ...
  async show(
    ...
    if (!appIcon) throw new Error("The packaged Control Plane icon is missing.");
