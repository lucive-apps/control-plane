    ...
    it.effect(
      ...
      () => {
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/com.lucive.ControlPlane.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "com.lucive.ControlPlane.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=Control Plane");
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.deepEqual(setPathMock.mock.calls, [
          ["userData", "/Users/alice/Library/Application Support/t3code"],
        ]);
      }).pipe(Effect.ensuring(Effect.sync(() => vi.unstubAllEnvs())));
    },
  );
