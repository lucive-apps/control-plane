...
describe("electron development launcher", () => {
  it("uses captured values only as fallbacks for a live runner environment", () => {
    ...
    assert.include(
      environmentScript,
      "if [ -z \"${T3CODE_DESKTOP_STATIC_RENDERER:-}\" ]; then export T3CODE_DESKTOP_STATIC_RENDERER='1'; fi",
    );
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/Control Plane (Dev).app",
      "Control Plane (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "Control Plane (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Control Plane (Dev).app/Contents/MacOS/Control Plane (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Control Plane (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/Control Plane (Dev).app/Contents/MacOS/Electron'",
    );
    assert.notInclude(script, "node_modules/electron");
  });

  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("Control Plane (Dev) Launcher");
  ...
  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/Control Plane (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/Control Plane (Dev).app",
    ]);
  });
