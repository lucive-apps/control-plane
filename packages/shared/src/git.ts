...
export const WORKTREE_BRANCH_PREFIX = "cp";
export const LEGACY_WORKTREE_BRANCH_PREFIX = "t3code";
// Canonical form is `cp/<8 hex>`. Older builds generated `t3code/<8 hex>` and
// older mobile builds generated `t3code/<uuid>` via Crypto.randomUUID()
// (always RFC 4122 v4), so the matcher also accepts exactly that shape —
// version nibble `4`, variant nibble `[89ab]` — to keep those threads eligible
// for branch regeneration without loosening beyond what was ever generated.
const TEMP_WORKTREE_TOKEN_PATTERN =
  "(?:[0-9a-f]{8}|[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})";
const TEMP_WORKTREE_BRANCH_PATTERN = new RegExp(
  `^(?:${WORKTREE_BRANCH_PREFIX}|${LEGACY_WORKTREE_BRANCH_PREFIX})\\/${TEMP_WORKTREE_TOKEN_PATTERN}$`,
);

export function stripWorktreeBranchPrefix(refName: string): string {
  for (const prefix of [`${WORKTREE_BRANCH_PREFIX}/`, `${LEGACY_WORKTREE_BRANCH_PREFIX}/`]) {
